...
describe('ReactDOMInput', function() {
  ...
  it('should not set a value for submit buttons unnecessarily', function() {
    var stub = React.DOM.input( {type:"submit"} );
    var node = renderTextInput(stub);
  ...
  it('should warn with value and no onChange handler', function() {
    ...
    try {
      ...
      var link = new ReactLink('yolo', mocks.getMockFunction());
      React.renderComponent(React.DOM.input( {type:"text", valueLink:link} ), node);
      expect(console.warn.mock.calls.length).toBe(0);

      React.renderComponent(
        React.DOM.input( {type:"text", value:"zoink", onChange:mocks.getMockFunction()} ),
        node
      );
      expect(console.warn.mock.calls.length).toBe(0);

      React.renderComponent(
        React.DOM.input( {type:"text", value:"zoink", readOnly:true} ),
        node
      );
      expect(console.warn.mock.calls.length).toBe(0);

      React.renderComponent(React.DOM.input( {type:"text", value:"zoink"} ), node);
      expect(console.warn.mock.calls.length).toBe(1);

      React.renderComponent(
        React.DOM.input( {type:"text", value:"zoink", readOnly:false} ),
        node
      );
